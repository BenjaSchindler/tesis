#!/usr/bin/env python3
"""
Experiment 03: K Neighbors Validation

Tests different K values for prompt support: [5, 10, 15, 25, 50, 75, 100]
Metrics: Macro F1, Acceptance Rate, Context Quality

Output: tab:k_neighbors for Metodologia.tex
"""
...
K_VALUES = EXPERIMENT_PARAMS["k_neighbors"]["K_VALUES"]  # [5, 10, 15, 25, 50, 75, 100]
EXPERIMENT_NAME = "k_neighbors"
...
def assess_context_quality(k: int) -> str:
    """Assess context quality based on K value."""
    if k <= 5:
        return "Insuficiente"
    elif k <= 10:
        return "Limitado"
...
def generate_synthetic_with_k(
    ...
    for target_class in np.unique(labels):
        ...
        for c_id in range(n_clusters):
            ...
            # Use K neighbors as examples in prompt
...
def run_k_neighbors_experiment() -> list:
    """Run K neighbors validation experiment."""
    print("\n" + "="*60)
    print("  EXPERIMENT 03: K NEIGHBORS VALIDATION")
    print("="*60)
...
def generate_latex_table(results: list) -> str:
    """Generate LaTeX table for K neighbors."""
    latex = r"""
\begin{table}[h]
\centering
...
"""
    ...
    for i, r in enumerate(results):
        if i == best_idx:
            latex += f"\\textbf{{{r.k_value}}} & \\textbf{{{r.macro_f1:.4f}}} & \\textbf{{{r.acceptance_rate*100:.0f}\\%}} & \\textbf{{{r.context_quality}}} \\\\\n"
        else:
            latex += f"{r.k_value} & {r.macro_f1:.4f} & {r.acceptance_rate*100:.0f}\\% & {r.context_quality} \\\\\n"
...
def main():
    ...
    print("\n" + "="*60)
    print("  SUMMARY: K Neighbors Results")
    print("="*60)
    for r in results:
        print(f"K={r.k_value:>3} | F1={r.macro_f1:.4f} | Acc={r.acceptance_rate*100:.0f}% | {r.context_quality} | Delta={r.delta_pct:+.2f}%")
