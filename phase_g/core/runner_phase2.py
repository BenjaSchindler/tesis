...
# Add parent directories to path for embedding_cache import
_script_dir = Path(__file__).parent.resolve()
for _potential_path in [_script_dir.parent / "phase_e", _script_dir]:
    if _potential_path.exists():
        sys.path.insert(0, str(_potential_path))
...
# ============================================================================
# Phase E: Embedding Cache for faster experiments
# ============================================================================
try:
    ...
    # Don't warn - cache is optional and only useful for batch experiments
...
# ============================================================================
# Phase F: LLM Generation Details Tracking
# ============================================================================

# Module-level storage for generation details (timing, tokens, prompts)
_generation_details: List[Dict[str, Any]] = []

def clear_generation_details() -> None:
    """Clear accumulated generation details (call before each experiment)."""
    global _generation_details
    _generation_details = []

def get_generation_details() -> List[Dict[str, Any]]:
    """Get accumulated generation details for metrics export."""
    return _generation_details.copy()

def add_generation_detail(detail: Dict[str, Any]) -> None:
    """Add a generation detail record."""
    _generation_details.append(detail)
...
def calculate_enhanced_budget(
    ...
    # Phase E: IP scaling parameters
    use_ip_scaling: bool = False,
    ip_threshold: float = 0.7,
    ip_boost_factor: float = 1.0,
    ip_minimum_base: int = 10,
    ip_minimum_scale: float = 20.0
) -> Tuple[int, str, Dict[str, float]]:
    """
    Phase 2 enhanced budget calculator with purity, F1, and IP multipliers.

    budget = base_budget × quality_mult × purity_mult × f1_mult × ip_mult
    ...
    Args:
        ...
        use_ip_scaling: Enable IP-based budget boost
        ip_threshold: IP threshold for boost activation
        ip_boost_factor: Multiplier factor for high-IP classes
        ip_minimum_base: Base minimum for high-IP classes
        ip_minimum_scale: Scale factor for IP minimum
    ...
    # 4. IP (Improvement Potential) multiplier (Phase E NEW)
    ip = 1.0 - baseline_f1  # Improvement Potential = 1 - F1
    ip_mult = 1.0
    ip_reason = f"IP={ip:.3f}"
    ip_minimum = 0

    if use_ip_scaling:
        if ip > ip_threshold:
            ip_mult = 1.0 + ip_boost_factor * ip
            ip_minimum = int(ip_minimum_base + ip_minimum_scale * ip)
            ip_reason = f"📈 IP={ip:.3f} (high potential) → {int(ip_mult*100)}% boost, min={ip_minimum}"
        elif ip > 0.5:
            ip_mult = 1.0 + 0.5 * ip_boost_factor * ip
            ip_reason = f"📊 IP={ip:.3f} (moderate) → {int(ip_mult*100)}%"
        else:
            ip_reason = f"✅ IP={ip:.3f} (low potential, no boost)"

    # Combine multipliers
    total_mult = quality_mult * purity_mult * f1_mult * ip_mult
    budget = max(10, int(base_budget * total_mult))

    # Apply IP minimum rescue for high-IP classes
    if use_ip_scaling and ip > ip_threshold and budget < ip_minimum:
        budget = ip_minimum
...
class PromptSpec:
    ...
    length_aware: bool = False  # Phase E: Generate texts with target word count
    target_word_count: int = 500  # Target word count for length-aware generation
    # Phase E: Enhanced quality logging fields
    anchor_purity: float = 1.0  # Purity score of the anchor's cluster (0-1)
    anchor_quality: float = 1.0  # Quality score of the anchor sample (0-1)
    ...
    def build_prompt(self) -> str:
        ...
        # Phase E: Add length-aware generation instructions
        if self.length_aware:
            min_words = int(self.target_word_count * 0.8)
            max_words = int(self.target_word_count * 1.2)
            length_instruction = (
                f"\n\n⚠️ CRITICAL LENGTH REQUIREMENT:\n"
                f"Each text MUST be approximately {self.target_word_count} words long "
                f"(minimum {min_words}, maximum {max_words} words).\n"
                f"This is ESSENTIAL - short texts (under 100 words) will be REJECTED.\n"
                f"Write detailed, elaborate texts that match the typical length of real data."
            )
            prompt = prompt + length_instruction
...
def calculate_class_word_counts(texts: Sequence[str]) -> Dict[str, float]:
    """
    Calculate average word count for texts.

    Returns:
        Dict with 'mean', 'median', 'std', 'min', 'max' word counts
    """
    word_counts = [len(text.split()) for text in texts]
    if not word_counts:
        return {'mean': 500, 'median': 500, 'std': 0, 'min': 0, 'max': 0}
...
def build_prompt_specs(
    ...
    class_word_count_target: Optional[int] = None,  # Phase E: Class-specific word count
) -> List[PromptSpec]:
    ...
    for cluster_id in range(n_clusters):
        ...
        # Phase E: Calculate cluster purity (mean intra-cluster similarity to centroid)
        cluster_purity = float(np.mean(sims))

        for anchor_idx, neighbour_idx, synthetic_emb in pairs:
            # Phase E: Calculate anchor quality (similarity of anchor to centroid)
            anchor_quality_score = float(sims[anchor_idx])
            anchor_text = cluster_texts[anchor_idx]
            neighbor_text = cluster_texts[neighbour_idx]
            # V3 Phase 1 - Mejora #3: K-NN ponderado por distancia
            # Phase G Extended: Use --n-shot to control number of examples (0=zero-shot)
            n_shot = getattr(args, 'n_shot', 5)
            if n_shot == 0:
                support_examples = []  # Zero-shot: no examples
            elif args.knn_support > 0:
                k_support = min(args.knn_support, len(cluster_texts), n_shot)
                support_examples = get_weighted_knn_support(synthetic_emb, cluster_matrix, cluster_texts, k=k_support)
            else:
                k_support = min(n_shot, max(3, len(cluster_texts) // 5))  # Use n_shot as max
                support_examples = get_weighted_knn_support(synthetic_emb, cluster_matrix, cluster_texts, k=k_support)
            ...
            prompt_specs.append(
                PromptSpec(
                    ...
                    length_aware=getattr(args, 'length_aware', False),
                    # Phase E: Use class-specific word count if provided, otherwise use global target
                    target_word_count=class_word_count_target if class_word_count_target else getattr(args, 'length_target_words', 500),
                    anchor_purity=cluster_purity,
...
def call_llm_batch(
    ...
    output_verbosity: str = "medium",
    max_tokens: Optional[int] = None,
) -> Dict[int, List[str]]:
    """
    Generate text samples using OpenAI API.

    For GPT-5-mini models:
        Uses Responses API with reasoning/verbosity parameters
    For other models (gpt-4o, gpt-4o-mini, etc.):
        Uses Chat Completions API with temperature/top_p parameters

    Args:
        max_tokens: Optional max tokens for response (Phase E: length-aware mode)
    """
    ...
    for spec in batch:
        ...
        for _ in range(max_retries):
            try:
                if is_gpt5_mini:
                    # GPT-5-mini: Use Responses API with reasoning/verbosity
                    system_content = (
                    ...
                    if output_verbosity and output_verbosity != "none":
                        api_params["text"] = {"verbosity": output_verbosity}

                    # Add reasoning effort if not "none"
                    if reasoning_effort and reasoning_effort != "none":
                        api_params["reasoning"] = {"effort": reasoning_effort}

                    # Phase F: Track timing
                    start_time = time.time()
                    response = client.responses.create(**api_params)
                    response_time_ms = int((time.time() - start_time) * 1000)

                    # Phase F: Capture token usage (Responses API)
                    tokens_used = {
                    ...
                    # Phase F: Log generation details
                    add_generation_detail({
                        "target_class": spec.target_class,
                        "cluster_id": getattr(spec, 'cluster_id', None),
                        "prompt_length": len(prompt),
                        "anchor_text_preview": spec.anchor_text[:100] if spec.anchor_text else "",
                        "n_samples_requested": spec.n_samples,
                        "n_samples_generated": len(candidates),
                    ...
                    # Traditional Chat Completions API (gpt-4o, gpt-4o-mini, etc.)
                    messages = [
                        {
                            "role": "system",
                            "content": (
                    ...
                    # Phase E: Add max_tokens for length-aware generation
                    ...
                    # Phase F: Capture token usage (Chat Completions API)
                    tokens_used = {
...
def filter_candidates(
    ...
    # Phase E: Enhanced quality logging parameters
    cluster_id: int = 0,
    anchor_purity: float = 1.0,
    anchor_quality: float = 1.0,
    target_word_count: int = 0,  # 0 means not specified
    ...
    # Helper to track rejection
    def track_rejection(reason: str):
        rejected[reason] += 1
        if quality_metrics is not None:
            quality_metrics["rejection_stats"][reason] += 1

    if debug:
        max_conf_str = f", conf_max={getattr(args, 'max_classifier_confidence', 1.0):.2f}" if getattr(args, 'max_classifier_confidence', 1.0) < 1.0 else ""
        print(
            f"Umbrales {target_class}: sim_floor={similarity_floor:.2f}, sim_th={sim_threshold:.2f}, "
            f"to_anchor={args.similarity_to_anchor:.2f}, conf_min={conf_threshold:.2f}{max_conf_str}"
        )
    ...
    for text, emb in zip(candidates, candidate_embeddings):
        ...
        if args.filter_mode in ("classifier", "hybrid"):
            ...
            if sim < effective_sim_threshold and max(sim_anchor, sim_neighbor) < similarity_to_anchor:
                ...
                # Phase F: Also check max confidence threshold (reject overly confident)
                max_conf = getattr(args, 'max_classifier_confidence', 1.0)
                if pred_class == target_class and proba >= conf_threshold and proba <= max_conf and margin_ok:
                    clf_ok = True
                elif proba > max_conf:
                    if debug:
                        rejected["high_conf"] = rejected.get("high_conf", 0) + 1
                    track_rejection("high_conf")
        ...
        if any(jaccard_similarity(text, acc, args.duplicate_ngram) > args.duplicate_threshold for acc in accepted_texts):
            ...
            track_rejection("dup_batch")
            continue

        # ===== ACCEPTED: Record quality metrics =====
        ...
        # Track per-sample quality metrics (if enabled)
        if quality_metrics is not None:
            # Compute KNN similarity if available
            knn_sim = None
            if nn_model is not None:
                try:
                    n_dist, _ = nn_model.kneighbors(emb.reshape(1, -1), return_distance=True)
                    sims_knn = 1.0 - n_dist.reshape(-1)
                    knn_sim = float(sims_knn.mean())
                except Exception:
                    knn_sim = None

            # Phase E: Calculate length match score if target word count is specified
            length_match_score = None
            if target_word_count > 0:
                # Score is 1.0 when exact match, decreases proportionally
                length_ratio = token_count / target_word_count
                length_match_score = max(0.0, 1.0 - abs(1.0 - length_ratio))

            sample_metrics = {
                "similarity_to_centroid": sim,
                "similarity_to_anchor": sim_anchor if anchor_emb is not None else None,
                "similarity_to_neighbor": sim_neighbor if neighbor_emb is not None else None,
                "knn_similarity": knn_sim,
                "classifier_confidence": conf,
                "token_count": token_count,
                # Phase E: Enhanced quality columns
                "cluster_id": cluster_id,
                "anchor_purity": anchor_purity,
                "anchor_quality": anchor_quality,
                "length_match_score": length_match_score,
                "target_word_count": target_word_count if target_word_count > 0 else None,
            }
            quality_metrics["per_sample"].append(sample_metrics)

    # Finalize quality metrics
    if quality_metrics is not None:
        quality_metrics["total_accepted"] = len(accepted_texts)
        quality_metrics["acceptance_rate"] = (
            len(accepted_texts) / quality_metrics["total_candidates"]
            if quality_metrics["total_candidates"] > 0 else 0.0
        )
...
def augment_class(
    ...
    # ========================================================================
    # Phase G Extended: Check for bypass conditions
    force_generation_classes = []
    if hasattr(args, 'force_generation_classes') and args.force_generation_classes:
        force_generation_classes = [c.strip().upper() for c in args.force_generation_classes.split(',')]

    gate_bypass = (
        (hasattr(args, 'disable_quality_gate') and args.disable_quality_gate) or
        (class_name.upper() in force_generation_classes)
    )

    if gate_bypass:
        print(f"🚀 GATE BYPASS: {class_name} - quality gates disabled (force generation)")

    if (args.enable_anchor_gate and ANCHOR_QUALITY_AVAILABLE and
        all_train_embeddings is not None and all_train_labels is not None and not gate_bypass):
        ...
        if not should_generate:
            ...
            return [], [], [], None  # Phase E: Return 4 values for quality metrics
        else:
    ...
    # Phase E: Calculate class-specific word count target if enabled
    class_word_count_target = None
    if getattr(args, 'use_class_length_average', False) and getattr(args, 'length_aware', False):
        word_stats = calculate_class_word_counts(class_texts)
        class_word_count_target = int(word_stats['mean'])
        print(f"📏 LENGTH: {class_name} - avg={word_stats['mean']:.0f} words (target={class_word_count_target})")
    ...
    prompt_specs = build_prompt_specs(
        ...
        class_word_count_target=class_word_count_target,
    )
    if not prompt_specs:
        return [], [], [], None
    ...
    # Quality metrics tracking (always enabled for detailed synthetic analysis)
    enable_quality_logging = True  # Phase E: Always track quality metrics
    ...
    for batch in tqdm(
        ...
        # Phase E/F: Determine max_tokens for LLM generation
        # Priority: max_completion_tokens (explicit) > llm_max_tokens (if length_aware)
        llm_max_tokens = getattr(args, 'max_completion_tokens', None)
        if llm_max_tokens is None and getattr(args, 'length_aware', False):
        ...
        for spec in batch:
            ...
            accepted_texts, accepted_embs, accepted_confs, batch_quality = filter_candidates(
                ...
                anchor_purity=spec.anchor_purity,
                anchor_quality=spec.anchor_quality,
                target_word_count=spec.target_word_count if spec.length_aware else 0,
            )
    ...
    # Finalize quality metrics
    if class_quality_metrics is not None:
        total_cand = class_quality_metrics["total_candidates"]
        total_acc = class_quality_metrics["total_accepted"]
        class_quality_metrics["acceptance_rate"] = (
            total_acc / total_cand if total_cand > 0 else 0.0
        )
        # Convert Counter to dict for JSON serialization
        class_quality_metrics["rejection_stats"] = dict(class_quality_metrics["rejection_stats"])
...
def orchestrate_pipeline(args: argparse.Namespace) -> None:
    ...
    # Phase F: Clear generation details at start of experiment
    clear_generation_details()
    ...
    # Phase E: Use embedding cache if available and configured
    cache = None
    if EMBEDDING_CACHE_AVAILABLE and getattr(args, "cache_dir", None):
        cache = EmbeddingCache(args.cache_dir)
        print(f"[Cache] Embedding cache enabled: {args.cache_dir}")

        # Get val_size for cache key consistency
        val_size = getattr(args, "val_size", None)

        train_embeddings = get_or_compute_embeddings(
            cache, embedder, train_proc,
    ...
    if getattr(args, 'auto_params', False):
        try:
            from auto_params import analyze_dataset, compute_auto_params, apply_auto_params
            profile = analyze_dataset(train_df["label"].values)
            auto = compute_auto_params(profile, verbose=True)
            apply_auto_params(args, auto)
        except ImportError:
            print("⚠️  auto_params module not found, using default parameters")

    # Phase F: Auto-adjust anchor quality threshold based on dataset quality scores
    if getattr(args, 'auto_anchor_threshold', False) and getattr(args, 'enable_anchor_gate', False):
        from anchor_quality_improvements import check_anchor_quality

        print(f"\n{'='*70}")
        print(f"Phase F: Auto-Adjust Anchor Quality Threshold")
        print(f"{'='*70}")
        print(f"Computing quality scores for all classes...")

        quality_scores = []
        for cls in label_encoder.classes_:
            cls_idx = list(label_encoder.classes_).index(cls)
            mask = y_train == cls_idx
            cls_embeddings = train_embeddings[mask]

            if len(cls_embeddings) < 3:
                print(f"  {cls}: skipped (n={len(cls_embeddings)} < 3)")
                continue

            quality, _, components = check_anchor_quality(
                embeddings=train_embeddings,
                labels=y_train,
                target_class=cls_idx,
                k=min(10, len(cls_embeddings) - 1),
                quality_threshold=0.0,  # No threshold for measurement
                cohesion_weight=getattr(args, 'anchor_cohesion_weight', 0.4),
                purity_weight=getattr(args, 'anchor_purity_weight', 0.4),
                separation_weight=getattr(args, 'anchor_separation_weight', 0.2)
            )
            quality_scores.append(quality)
            print(f"  {cls}: {quality:.3f} (coh={components['cohesion']:.3f}, pur={components['knn_purity']:.3f}, sep={components['separation']:.3f})")

        if quality_scores:
            median_quality = np.median(quality_scores)
            margin = getattr(args, 'auto_anchor_margin', 0.02)
            auto_threshold = max(0.0, median_quality - margin)

            print(f"\n  Median quality: {median_quality:.3f}")
            print(f"  Margin: {margin:.3f}")
            print(f"  Auto threshold: {auto_threshold:.3f} (median - margin)")
            print(f"  Original threshold: {args.anchor_quality_threshold:.3f}")

            # Override the manual threshold
            args.anchor_quality_threshold = auto_threshold
            print(f"  ✓ Using auto-threshold: {auto_threshold:.3f}")
        else:
            print("  ⚠️  No classes had enough samples, keeping original threshold")
        print()
    ...
    # Per-class quality metrics for diagnostic logging
    per_class_quality_metrics: Dict[str, Dict] = {}
    # Per-sample quality metrics (list of dicts, one per synthetic)
    per_sample_quality: List[Dict] = []
    ...
    for cls in target_classes:
        ...
        # Initial cluster estimate (will be adjusted after purity check)
        ...
        # Phase E: Purity-aware cluster adjustment
        # Low purity + small samples = use fewer clusters to concentrate good anchors
        min_cluster_samples = getattr(args, 'min_cluster_samples', 5)
        if purity < 0.03 or n_samples < 30:
            # Very low purity or tiny class: limit clusters
            max_clusters_for_size = max(3, n_samples // min_cluster_samples)
            if n_clusters_est > max_clusters_for_size:
                old_clusters = n_clusters_est
                n_clusters_est = max_clusters_for_size
                print(f"  ⚠️ Purity-aware: {cls} clusters {old_clusters}→{n_clusters_est} (purity={purity:.3f}, n={n_samples})")
        ...
        # Phase E: Apply IP (Improvement Potential) scaling
        if getattr(args, 'use_ip_scaling', False):
            ip = 1.0 - baseline_f1  # Improvement Potential
            ip_threshold = getattr(args, 'ip_threshold', 0.7)
            ip_boost_factor = getattr(args, 'ip_boost_factor', 1.0)
            ip_minimum_base = getattr(args, 'ip_minimum_base', 10)
            ip_minimum_scale = getattr(args, 'ip_minimum_scale', 20.0)

            if ip > ip_threshold:
                ip_mult = 1.0 + ip_boost_factor * ip
                ip_minimum = int(ip_minimum_base + ip_minimum_scale * ip)
                old_budget = dynamic_budget
                dynamic_budget = int(dynamic_budget * ip_mult)
                if dynamic_budget < ip_minimum:
                    dynamic_budget = ip_minimum
                args.cap_class_abs = dynamic_budget
                print(f"   📈 Phase E IP Scaling: IP={ip:.3f} (>{ip_threshold}) → {int(ip_mult*100)}% boost")
                print(f"      Budget: {old_budget} → {dynamic_budget} (min={ip_minimum})")
            elif ip > 0.5:
                ip_mult = 1.0 + 0.5 * ip_boost_factor * ip
                old_budget = dynamic_budget
                dynamic_budget = int(dynamic_budget * ip_mult)
                args.cap_class_abs = dynamic_budget
                print(f"   📊 Phase E IP Scaling: IP={ip:.3f} (moderate) → {int(ip_mult*100)}%")
                print(f"      Budget: {old_budget} → {dynamic_budget}")

        # === Generate synthetics ===
        synth_count_before = len(synthetic_texts)
        texts, embs, confs, class_quality = augment_class(
            client,
            embedder,
            baseline_model,
            label_encoder,
        ...
        # Store quality metrics AFTER validation gating (only for accepted synthetics)
        if class_quality is not None and len(texts) > 0:
            # If val gating rejected, we need to remove the quality metrics too
            # For now, only add if texts is not empty (after potential val rejection)
            per_class_quality_metrics[cls] = class_quality
            # Add per-sample quality with class label (only for accepted samples)
            num_accepted = len(texts)
            for i, sample_q in enumerate(class_quality.get("per_sample", [])[:num_accepted]):
                sample_q["label"] = cls
                per_sample_quality.append(sample_q)
        ...
        if prediction_decision is not None:
            prediction_results.append({
                ...
                "n_clusters": n_clusters_est,
                "quality_score": quality_score,
                "anchor_purity": purity,  # Phase E: Include anchor metrics
                "anchor_cohesion": cohesion,  # Phase E: Include anchor metrics
    ...
    # Phase E: Always build DataFrame with quality metrics (not just verbose_logging)
    if per_sample_quality:
        # CRITICAL: Ensure lengths match before creating DataFrame
        num_synthetics = len(synthetic_labels)
        num_texts = len(synthetic_texts)
        num_quality_entries = len(per_sample_quality)

        # ASSERTION 1: synthetic_labels and synthetic_texts must have same length
        assert num_synthetics == num_texts, \
            f"FATAL: synthetic_labels ({num_synthetics}) and synthetic_texts ({num_texts}) length mismatch!"

        if num_synthetics != num_quality_entries:
            print(f"⚠️  WARNING: Length mismatch - {num_synthetics} synthetics but {num_quality_entries} quality entries")
            print(f"   Adjusting per_sample_quality to match synthetic count...")

            if num_quality_entries > num_synthetics:
                # Truncate excess quality entries
                per_sample_quality = per_sample_quality[:num_synthetics]
                print(f"   Truncated to {num_synthetics} entries")
            else:
                # Pad with empty dicts for missing quality entries
                padding_needed = num_synthetics - num_quality_entries
                empty_quality = {
                    "similarity_to_centroid": None,
                    "similarity_to_anchor": None,
                    "similarity_to_neighbor": None,
                    "knn_similarity": None,
                ...
                for _ in range(padding_needed):
                    per_sample_quality.append(empty_quality.copy())
                print(f"   Padded with {padding_needed} empty quality entries")

        # ASSERTION 2: After adjustment, lengths must match
        assert len(per_sample_quality) == num_synthetics, \
            f"FATAL: After adjustment, per_sample_quality ({len(per_sample_quality)}) != synthetics ({num_synthetics})!"

        # Create extended DataFrame with quality metrics columns
        df_data = {"label": synthetic_labels, "text": synthetic_texts}

        # Extract quality metric columns from per_sample_quality list
        # Now guaranteed to have same length as synthetic_labels
        # Phase E: Added cluster_id, anchor_purity, anchor_quality, length_match_score
        quality_cols = ["similarity_to_centroid", "similarity_to_anchor", "similarity_to_neighbor",
                        "knn_similarity", "classifier_confidence", "token_count",
                        "cluster_id", "anchor_purity", "anchor_quality", "length_match_score"]

        for col in quality_cols:
            values = [sq.get(col) for sq in per_sample_quality]
            # ASSERTION 3: Each quality column must have same length as synthetics
            assert len(values) == num_synthetics, \
                f"FATAL: Quality column '{col}' has {len(values)} values but expected {num_synthetics}!"
            # Only add column if we have data
            if any(v is not None for v in values):
                df_data[col] = values

        # ASSERTION 4: All DataFrame columns must have same length before creation
        col_lengths = {k: len(v) for k, v in df_data.items()}
        if len(set(col_lengths.values())) > 1:
            print(f"❌ FATAL: DataFrame columns have different lengths: {col_lengths}")
            raise ValueError(f"DataFrame column length mismatch: {col_lengths}")

        synthetic_df = pd.DataFrame(df_data)
        print(f"Synthetic CSV includes {len(quality_cols)} quality metric columns (verbose_logging=True)")
        print(f"✓ DataFrame created successfully: {len(synthetic_df)} rows, {len(synthetic_df.columns)} columns")
    else:
    ...
    metrics_dict = {
        ...
        "synthetic_data": {
            ...
            "per_class_details": {},  # Phase E: Will be populated below
        },
    }

    # Phase E: Add detailed per-class synthetic information
    for pred_info in prediction_results:
        cls = pred_info.get("class", "UNKNOWN")
        metrics_dict["synthetic_data"]["per_class_details"][cls] = {
            "n_samples": pred_info.get("n_samples", 0),
            "baseline_f1": pred_info.get("baseline_f1", 0.0),
            "quality_score": pred_info.get("quality_score", 0.0),
            "anchor_purity": pred_info.get("anchor_purity", 0.0),
            "anchor_cohesion": pred_info.get("anchor_cohesion", 0.0),
            "synthetics_generated": pred_info.get("synthetics_generated", 0),
            "decision": pred_info.get("decision", "unknown"),
    ...
    # Phase E: Always add per-class quality metrics (not just with verbose_logging)
    if per_class_quality_metrics:
        # Build rejection analysis summary
        total_rejected = sum(
            sum(pq.get("rejection_stats", {}).values())
            for pq in per_class_quality_metrics.values()
        )
        total_candidates = sum(
            pq.get("total_candidates", 0)
            for pq in per_class_quality_metrics.values()
        )
        total_accepted_all = sum(
            pq.get("total_accepted", 0)
            for pq in per_class_quality_metrics.values()
        )

        # Aggregate rejection reasons
        all_rejection_reasons = {}
        for cls, pq in per_class_quality_metrics.items():
            for reason, count in pq.get("rejection_stats", {}).items():
                all_rejection_reasons[reason] = all_rejection_reasons.get(reason, 0) + count

        metrics_dict["per_class_quality"] = per_class_quality_metrics
        metrics_dict["rejection_analysis"] = {
            "total_generated": total_candidates,
            "total_accepted": total_accepted_all,
            "total_rejected": total_rejected,
            "overall_acceptance_rate": total_accepted_all / total_candidates if total_candidates > 0 else 0.0,
            "rejection_reasons": all_rejection_reasons,
            "per_class_acceptance_rate": {
                cls: pq.get("acceptance_rate", 0.0)
                for cls, pq in per_class_quality_metrics.items()
            }
        }
        print(f"📊 Metrics JSON includes per_class_quality and rejection_analysis")

    # Phase F: Add generation details (timing, tokens, prompts)
    generation_details = get_generation_details()
    if generation_details:
        # Compute summary statistics
        total_llm_time_ms = sum(d.get("llm_response_time_ms", 0) for d in generation_details)
        total_tokens = sum(
            d.get("tokens_used", {}).get("total_tokens", 0) or
            (d.get("tokens_used", {}).get("input_tokens", 0) + d.get("tokens_used", {}).get("output_tokens", 0))
            for d in generation_details
        )
        metrics_dict["generation_details"] = {
            "summary": {
                "total_llm_calls": len(generation_details),
                "total_llm_time_ms": total_llm_time_ms,
                "avg_llm_time_ms": total_llm_time_ms / len(generation_details) if generation_details else 0,
                "total_tokens_used": total_tokens,
                "model_used": generation_details[0].get("model", "unknown") if generation_details else "unknown",
            },
            "per_call": generation_details,  # Full details for analysis
        }
        print(f"📊 Metrics JSON includes generation_details ({len(generation_details)} LLM calls, {total_llm_time_ms/1000:.1f}s total)")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cache-dir", default=None, help="Directory for embedding cache (enables caching if set)")
    ...
    parser.add_argument("--auto-params", action="store_true", help="Phase E: Auto-adjust parameters based on dataset characteristics")
    ...
    parser.add_argument("--reasoning-effort",
                        ...
                        default="none",
                        help="GPT-5-mini reasoning effort (none=fastest, low/medium/high=increasing reasoning quality)")
    parser.add_argument("--output-verbosity",
                        choices=["none", "low", "medium", "high"],
                        default="medium",
                        help="GPT-5-mini output verbosity (none=no param, low/medium/high)")
    ...
    parser.add_argument("--max-classifier-confidence", type=float, default=1.0, help="Phase F: Upper bound for classifier confidence (reject overly confident)")
    ...
    # Phase G Extended: Aggressive options for problematic classes
    parser.add_argument("--disable-quality-gate", action="store_true",
                        help="Phase G Extended: Disable ALL quality gates entirely (for ESFP/ESTJ experiments)")
    parser.add_argument("--force-generation-classes", type=str, default="",
                        help="Phase G Extended: Comma-separated list of classes to force generation (bypass gates)")
    parser.add_argument("--custom-prompt-file", type=str, default=None,
                        help="Phase G Extended: Path to custom prompt file for specialized generation")
    parser.add_argument("--n-shot", type=int, default=5,
                        help="Phase G Extended: Number of examples in prompt (0=zero-shot, 3=few-shot, etc.)")

    # Auto-adjust anchor quality threshold (Phase F)
    parser.add_argument("--auto-anchor-threshold", action="store_true", help="Auto-adjust anchor quality threshold based on dataset median")
    parser.add_argument("--auto-anchor-margin", type=float, default=0.02, help="Margin below median for auto threshold (default: 0.02)")
    ...
    parser.add_argument("--length-aware", action="store_true",
                        help="Phase E: Enable length-aware generation (target ~500 words per synthetic text)")
    parser.add_argument("--length-target-words", type=int, default=500,
                        help="Phase E: Target word count for length-aware generation (default: 500)")
    parser.add_argument("--llm-max-tokens", type=int, default=4000,
                        help="Phase E: Max tokens for LLM response when length-aware is enabled")
    parser.add_argument("--max-completion-tokens", type=int, default=None,
                        help="Phase F: Max completion tokens for LLM response (overrides llm-max-tokens)")
    parser.add_argument("--use-class-length-average", action="store_true",
                        help="Phase E: Use class-specific average word count as target (calculated from training data)")
    parser.add_argument("--minority-f1-threshold", type=float, default=0.5,
                        help="Phase E: Classes with F1 below this are considered minority (for relaxed thresholds)")
    ...
    # Phase E: IP (Improvement Potential) scaling for minority classes
    parser.add_argument("--use-ip-scaling", action="store_true", help="Phase E: Enable IP-based budget scaling (boost low-F1 classes)")
    parser.add_argument("--ip-threshold", type=float, default=0.7, help="Phase E: IP threshold above which to boost budget (IP = 1 - baseline_f1)")
    parser.add_argument("--ip-boost-factor", type=float, default=1.0, help="Phase E: Boost multiplier for high-IP classes (budget × (1 + factor × IP))")
    parser.add_argument("--ip-minimum-base", type=int, default=10, help="Phase E: Base minimum synthetics for high-IP classes")
    parser.add_argument("--ip-minimum-scale", type=float, default=20.0, help="Phase E: Scale factor for IP minimum (min = base + scale × IP)")
    ...
    for seed in args.ensemble_seeds:
        ...
        for cls in target_classes:
            ...
            if texts:
                ...
                # Collect quality metrics for verbose logging
                if class_quality is not None and len(texts) > 0:
                    per_class_quality_metrics[cls] = class_quality
                    # CRITICAL: Slice to match number of accepted texts (defensive fix)
                    num_accepted = len(texts)
                    num_quality = len(class_quality.get("per_sample", []))

                    # Warn if mismatch detected
                    if num_quality != num_accepted:
                        print(f"⚠️  Multi-seed ablation: Quality mismatch for {cls} - {num_quality} quality entries but {num_accepted} accepted texts (slicing to match)")
