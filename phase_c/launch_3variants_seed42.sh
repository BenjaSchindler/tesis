#!/bin/bash
# Launch 3 Robustness Variants in Parallel (Seed 42)
# Variante A: 5 prompts/cluster
# Variante B: Multi-temperature ensemble
# Variante C: GPT-5-mini with reasoning
...
echo "═══════════════════════════════════════════════════════════"
echo "  Phase C - 3 Robustness Variants (Seed 42)"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "Testing 3 approaches to reduce LLM stochasticity:"
echo ""
...
echo ""
echo "  ©️  Variante C: GPT-5-mini (Responses API)"
echo "      - 3 prompts/cluster"
echo "      - gpt-5-mini-2025-08-07"
echo "      - reasoning=minimal, verbosity=medium"
echo "      - Uses new Responses API (more deterministic)"
echo ""
echo "Configuration:"
echo "  Seed: $SEED"
echo "  Machine Type: $MACHINE_TYPE + $GPU_TYPE"
echo "  Zone: $ZONE"
echo "  Estimated time: ~45 min per variant"
echo "  Estimated cost: ~$2.50 total"
echo ""
echo "═══════════════════════════════════════════════════════════"
echo ""

read -p "Launch all 3 variants in parallel? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Aborted."
    exit 1
...
echo ""
echo "Step 1: Creating 3 GPU VMs..."
echo ""

# Create VMs in parallel
for VM_NAME in "$VM_VARIANTE_A" "$VM_VARIANTE_B" "$VM_VARIANTE_C"; do
...
# Launch Variante C: GPT-5-mini (deterministic, fixed temp=1)
echo "©️  Launching Variante C (GPT-5-mini deterministic)..."
gcloud compute ssh "$VM_VARIANTE_C" --zone="$ZONE" --command="
    export OPENAI_API_KEY='$OPENAI_API_KEY'
    source venv/bin/activate
    ...
    nohup python3 -u runner_phase2.py \
        ...
        --contamination-threshold 0.95 \
        \
        --synthetic-output variante_c_seed42_synthetic.csv \
        --augmented-train-output variante_c_seed42_augmented.csv \
        --metrics-output variante_c_seed42_metrics.json \
        > variante_c_seed42.log 2>&1 &

    echo 'Variante C launched!'
    sleep 2
    sudo shutdown -h +90
" &
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ All 3 Variants Launched!"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "VMs running:"
echo "  🅰️  $VM_VARIANTE_A (5 prompts/cluster)"
echo "  🅱️  $VM_VARIANTE_B (multi-temp ensemble)"
echo "  ©️  $VM_VARIANTE_C (GPT-5-mini reasoning)"
echo ""
echo "Estimated completion: ~45 minutes"
echo "Auto-shutdown: Enabled (90 min after launch)"
echo ""
...
echo ""
echo "  # Variante C"
echo "  gcloud compute ssh $VM_VARIANTE_C --zone=$ZONE --command='tail -30 variante_c_seed42.log'"
echo ""
echo "  # GPU usage"
echo "  gcloud compute ssh $VM_VARIANTE_A --zone=$ZONE --command='nvidia-smi'"
echo ""
echo "Download results (after completion):"
echo "  ./collect_3variants_seed42.sh"
echo ""
echo "═══════════════════════════════════════════════════════════"
