...
def calculate_enhanced_budget(
    ...
    if purity < purity_low_threshold:
        ...
        purity_reason = f"Low purity ({purity:.3f})"
    else:
        purity_mult = 1.0
    ...
    if ip > ip_threshold:
        ...
        ip_reason = f"IP={ip:.3f} (high potential) boost"
    elif ip > 0.5:
        ip_mult = 1 + 0.5 * ip_boost_factor * ip
        ip_reason = f"IP={ip:.3f} (moderate)"
    else:
        ip_mult = 1.0
        ...
        for anchor_idx, neighbour_idx, synthetic_emb in pairs:
            ...
            prompt_specs.append(
                PromptSpec(
                    ...
                    target_word_count=getattr(args, 'length_target_words', 500),
                )
            )
    return prompt_specs
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--reasoning-effort",
                        ...
                        default="none",
                        help="GPT-5-mini reasoning effort (none=no param, minimal=fast with few reasoning tokens)")
