#!/usr/bin/env python3
"""
Experiment: RARE_MLP with n_shot=60

Compares MLP baseline with RARE_massive_oversample + n_shot=60.
This uses the SAME K-Fold setup as the original RARE_MLP experiments
to allow fair comparison of baselines.

Previous RARE_MLP baseline: 0.2075 (with default n_shot)
Previous MLP_nshot60 baseline: 0.2361 (different K-Fold)

This experiment will use RARE_MLP's K-Fold to get comparable results.
"""
...
def get_rare_massive_nshot60_params():
    """
    Get RARE_massive_oversample params with n_shot=60 override.
    """
    # Start with RARE_massive_oversample base
    params = {
...
def main():
    print("="*70)
    print("RARE_MLP with n_shot=60")
    print("="*70)
    print("\nThis experiment combines:")
    print("  - RARE_massive_oversample config (for rare classes)")
    print("  - n_shot=60 prompting (optimal from many-shot experiments)")
    print("  - MLP 512-256-128 classifier")
    print("  - SAME K-Fold setup as original RARE_MLP (seed=42)")
    print("\nExpected baseline should match original RARE_MLP: ~0.2075")

    # Load data
    print("\nLoading data...")
    texts, labels = load_data()
    texts = np.array(texts)  # Convert to numpy
    ...
    # Get params with n_shot=60
    params = get_rare_massive_nshot60_params()
    print(f"\nConfig params:")
    print(f"  n_shot: {params.get('n_shot')}")
    ...
    results["config_name"] = "RARE_MLP_nshot60"
    results["description"] = "RARE_massive_oversample + n_shot=60 + MLP 512-256-128"
    ...
    # Print results
    print("\n" + "="*70)
    print("RESULTS: RARE_MLP with n_shot=60")
    print("="*70)
    ...
    print(f"    Original RARE_MLP baseline:  0.2075")
    print(f"    This experiment baseline:    {results['baseline_mean']:.4f}")
    print(f"    Difference:                  {results['baseline_mean'] - 0.2075:+.4f}")

    # Save results
    results_dir = RESULTS_DIR / "rare_mlp_nshot60"
    results_dir.mkdir(parents=True, exist_ok=True)

    result_file = results_dir / "RARE_MLP_nshot60_kfold.json"
