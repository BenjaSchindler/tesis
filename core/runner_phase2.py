    ...
    for text, emb in zip(candidates, candidate_embeddings):
        ...
        if args.filter_mode in ("classifier", "hybrid"):
            ...
            if sim < effective_sim_threshold and max(sim_anchor, sim_neighbor) < similarity_to_anchor:
                ...
                # Check confidence band: min <= proba <= max
                max_conf = getattr(args, "max_classifier_confidence", 1.0)
                conf_in_band = proba >= conf_threshold and (max_conf >= 1.0 or proba <= max_conf)
                if pred_class == target_class and conf_in_band and margin_ok:
                    clf_ok = True
                elif max_conf < 1.0 and proba > max_conf:
                    if debug:
                        rejected["conf_too_high"] = rejected.get("conf_too_high", 0) + 1
                    track_rejection("conf_too_high")
...
def augment_class(
    ...
    all_train_embeddings: Optional[np.ndarray] = None,
    all_train_labels: Optional[np.ndarray] = None,
) -> Tuple[List[str], List[np.ndarray], List[float], Optional[Dict[str, Any]]]:
    """
...
def orchestrate_pipeline(args: argparse.Namespace) -> None:
    ...
    # Build DataFrame with quality metrics if verbose_logging enabled
    if getattr(args, 'verbose_logging', False) and per_sample_quality:
    ...
    # Add per-class quality metrics if verbose_logging enabled
    if getattr(args, 'verbose_logging', False) and per_class_quality_metrics:
        ...
        print(f"Metrics JSON includes per_class_quality and rejection_analysis (verbose_logging=True)")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--presence-penalty", type=float, default=0.0,
                        help="OpenAI presence_penalty (0.0-2.0) for diverse outputs")
    ...
    parser.add_argument("--max-classifier-confidence", type=float, default=1.0,
                        help="Max confidence threshold for band filtering (reject if proba > max)")
